...
class PostController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->category) {
            return PostResourse::collection(Category::where('name', $request->category)->firstOrFail()->posts()->latest()->paginate(2)->withQueryString());
        }else if ($request->search) {
            return  PostResourse::collection(Post::where('title', 'like', '%' . $request->search . '%')
                ->orWhere('body', 'like', '%' . $request->search . '%')->latest()->paginate(2)->withQueryString());
        }
        return PostResourse::collection(Post::latest()->paginate(2));
    }
